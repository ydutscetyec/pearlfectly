      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100vh-5rem)] max-w-6xl items-center justify-center">
        <div className="grid w-full overflow-hidden rounded-[2.75rem] border border-white/65 bg-[#FFF8EF]/72 shadow-2xl shadow-black/10 backdrop-blur-xl lg:grid-cols-[0.92fr_1.08fr]">
          <section className="relative hidden min-h-[640px] overflow-hidden border-r border-[#B89A5E]/18 bg-[#1B1411] p-10 text-[#FFF8EF] lg:block">
            <img src="/weekly-pearl-box.png" alt="Pearl jewelry packaging" className="absolute inset-0 h-full w-full object-cover opacity-38" />
            ...
            <div className="relative flex h-full flex-col justify-between">
              ...
              <div>
                ...
                <h2 className="font-serif text-6xl leading-[0.95] tracking-[-0.04em]">
                  A softer way to enter your pearl atelier.
                </h2>
                <p className="mt-6 max-w-md text-sm leading-7 text-[#FFF8EF]/72">
                  Your account keeps checkout personal, protects admin access, and keeps the boutique experience consistent from shop to dashboard.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-3 text-sm text-[#FFF8EF]/78">
                <div className="rounded-[1.5rem] border border-[#FFF8EF]/14 bg-[#FFF8EF]/10 p-4 backdrop-blur-md">
                  <ShieldCheck className="mb-3 h-5 w-5 text-[#D9C7A5]" /> Secure credentials
                </div>
                <div className="rounded-[1.5rem] border border-[#FFF8EF]/14 bg-[#FFF8EF]/10 p-4 backdrop-blur-md">
                  <Gem className="mb-3 h-5 w-5 text-[#D9C7A5]" /> Pearl profile
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="p-7 sm:p-10 lg:p-12">
            ...
            <div className="mb-8">
              <p className="mb-3 text-xs font-semibold uppercase tracking-[0.28em] text-[#8A6A3F]">Member Access</p>
...
function SignInContent() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const target = safeCallbackUrl(rawCallbackUrl, sessionData?.user);
  ...
  return (
    <AuthShell
      title="Sign in"
      subtitle="Access your Pearlfectly profile, checkout with your real customer details, and continue shopping smoothly."
      footer={
        <p className="mt-7 text-center text-sm text-[#4A3832]/70">
          New here?{" "}
          <a href={`/signup?callbackUrl=${encodeURIComponent(rawCallbackUrl)}`} className="font-semibold text-[#8A6A3F] underline underline-offset-4 hover:text-[#1B1411]">
            Create an account
          </a>
          .
        </p>
